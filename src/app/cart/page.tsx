            ...
            <div className="mt-8 grid gap-10 lg:grid-cols-[1fr_360px]">
                <div className="space-y-4">
                    {lines.map((line) => (
                        <div
                            key={line.productId}
                            className="flex gap-4 rounded-2xl bg-white p-4 shadow-soft sm:items-center"
                        >
                            <Link
                                href={`/products/${line.product.slug}`}
                                className="relative h-24 w-24 shrink-0 overflow-hidden rounded-xl bg-sage-50"
                            >
                                <img
                            ...
                            <div className="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    ...
                                    <Link
                                        href={`/products/${line.product.slug}`}
                                        className="font-display text-sm font-bold text-ink hover:text-sage-700"
                                    >
                                        {line.product.name}
                                    </Link>
                                    <p className="mt-1 text-sm font-bold text-clay-600">
                    ...
                    <Link
                        href="/products"
                        className="inline-flex items-center gap-1.5 text-sm font-semibold text-sage-700 hover:text-sage-800"
                    >
                ...
                <aside className="h-fit rounded-2xl bg-white p-6 shadow-soft">
                    ...
                    <div className="flex items-center justify-between py-5">
                        ...
                        <span className="font-display text-xl font-bold text-clay-600">
                            {currency(total)}
                        </span>
                    </div>
                    <Link href="/checkout">
